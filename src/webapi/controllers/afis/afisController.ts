...
class AfisController extends BaseController {
    protected config(): void {
        this.router.get("/:index/history",
        (req, res, next) => this.verifyToken(req, res, next),
        (req, res, next) => this.setCache(req, res, next, 1),
        (req, res) => this.getCareerAfisHistory(req as CustomRequest, res)
    )

        this.router.get("/:index/:month",
            (req, res, next) => this.verifyToken(req, res, next),
            (req, res, next) => this.setCache(req, res, next, 1),
    ...
    async getCareerAfisByIndex(req: CustomRequest, res: Response) {
        try {
            ...
            const data = await afisDataSource.getAfisFromCareer(carrera, month, req.user, req.trim);
    ...
    async getCareerAfisHistory(req: CustomRequest, res: Response) {
        try {
            ...
            const data = await afisDataSource.getAfisHistoryFromCareer(carrera, req.user, req.trim);
